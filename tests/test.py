...
def test_for_attribute():
    ...
    assert (
        str(test_tag)
        == """<div class='my-class\' """ + """for='my-form'>inside the div</div>"""
    )
...
def test_inner_html_list():
    assert (
        str(TagFactory("div.my-class", [TagFactory("div", "child tag")]))
...
def test_omitted_dash():
    test_tag = TagFactory("div", "", role="application", ariadescribedby="info")
    assert (
        str(test_tag)
...
def test_add_child_element_with_multiple_child_tags():
    ...
    test_tag.add_child_element(
        [
            TagFactory(
                "div.container",
                TagFactory(
                    "div1", TagFactory("div2", TagFactory("div3", TagFactory("div4")))
                ),
            )
        ]
    )
    assert (
        str(test_tag)
        == """<test_tag><div class='container'><div1><div2>"""
...
def test_singleton_tag_as_child_element():
    a_tag = TagFactory(
        "a", SingletonTag("img", src="logo_w3s.gif"), href="www.google.com"
    )
    assert (
        str(a_tag)
